...
const base64ToFile = (base64String, filename, mimeType = 'application/octet-stream') => {
...
if (file.type.startsWith('image/')) {
  ...
  setBlobUrls(prev => ({ ...prev, [fieldName]: previewUrl }));
}
};
...
try {
  const employeeSerialNumber = formData.employeeSerialNumber

  // Create FormData for multipart upload
  const formDataObj = new FormData()
  
  // Create user profile data without file fields
  const userProfileData = { ...formData }
  const fileFields = [
    'panFile', 'adhaarFile', 'passbookFile', 'photo', 
    'qualificationDocument', 'offerLetter', 'addressProofFile',
    'medicalBackgroundDocument', 'legalBackgroundDocument'
  ]
  
  // Remove file fields from user profile data
  fileFields.forEach(field => {
    delete userProfileData[field]
  })

  // Append user profile as JSON string
  formDataObj.append('userProfile', JSON.stringify(userProfileData))

  // Handle file uploads - only append files that have been changed or are File objects
  fileFields.forEach(fieldName => {
    ...
    if (fileData) {
      if (fileData instanceof File) {
        // New file upload
        formDataObj.append(fieldName, fileData)
        console.log(`Appending new file for ${fieldName}:`, fileData.name)
      } else if (fileChanges[fieldName] && typeof fileData === 'string') {
        // Convert base64 to File if it was changed
        const file = base64ToFile(fileData, `${fieldName}.pdf`)
        if (file) {
          formDataObj.append(fieldName, file)
          console.log(`Appending converted file for ${fieldName}`)
        }
      }
      // If it's unchanged base64 data, don't append it
    }
  })

  console.log("Updating user data for Employee Serial Number:", employeeSerialNumber)

  const response = await fetch(`http://localhost:8080/api/user-profiles/update/employee/${employeeSerialNumber}`, {
    method: "PUT",
    body: formDataObj,
  })
...
Status: user.status === "Y" ? "Onboarded" : user.status === "R" ? "Pending for Update Approval" : "Onboarding Pending",
}))
...
return <span className="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Pending for Update Approval</span>
} else {
...
const url = createBlobUrl(fileData, contentType);
...
// Use cached URL if available for base64 data
if (blobUrls.photo) return blobUrls.photo;

const url = createBlobUrl(photoData, 'image/jpeg');
                ...
                <div className="grid gap-6 mb-6 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      disabled={!isEditing}
                    />
                    {(safeFormData.photo || fileChanges.photo) && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.panFile || fileChanges.panFile) && (
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.adhaarFile || fileChanges.adhaarFile) && (
                          <div className="text-xs text-green-600">
                            Document uploaded successfully
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.passbookFile || fileChanges.passbookFile) && (
                          <div className="text-xs text-green-600">
                            Document uploaded successfully
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.qualificationDocument || fileChanges.qualificationDocument) && (
                          <div className="text-xs text-green-600">
                            Document uploaded successfully
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.offerLetter || fileChanges.offerLetter) && (
                          <div className="text-xs text-green-600">
                            Document uploaded successfully
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.addressProofFile || fileChanges.addressProofFile) && (
                          <div className="text-xs text-green-600">
                            Document uploaded successfully
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.medicalBackgroundDocument || fileChanges.medicalBackgroundDocument) && (
                          <div className="text-xs text-green-600">
                            Document uploaded successfully
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          disabled={!isEditing}
                        />
                        {(safeFormData.legalBackgroundDocument || fileChanges.legalBackgroundDocument) && (
                          <div className="text-xs text-green-600">
                            Document uploaded successfully
                          </div>
                        )}
